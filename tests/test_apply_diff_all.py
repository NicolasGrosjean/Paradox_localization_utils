...
class TestApplyDiffAll(unittest.TestCase):
    ...
    def test_apply_diff_edited_line(self):
        ...
        self.assertEqual(lines[0].replace("\n", ""), "\ufeffl_french:")
        self.assertEqual(lines[1].replace("\n", ""), "  # Ideas")
        self.assertEqual(lines[2].replace("\n", ""), ' KEY50:1 "valeur0" #Comment to keep #ZX Commentaire à garder')
        self.assertEqual(lines[3].replace("\n", ""), "  ")
        self.assertEqual(lines[4].replace("\n", ""), "  # Events")
        self.assertEqual(lines[5].replace("\n", ""), '  KEY51:2 "valeur1" #ZX Un autre')
    ...
    def test_apply_diff_edited_line_keep_Z(self):
        with open(os.path.abspath(os.path.join(self.data_dir, "new", "16_l_french.yml")), "r", encoding="utf8") as f:
            lines = f.readlines()
        self.assertEqual(lines[0].replace("\n", ""), "\ufeffl_french:")
        self.assertEqual(lines[1].replace("\n", ""), '  KEY160:0 "valeur0"')
        self.assertEqual(lines[2].replace("\n", ""), ' KEY161:0Z "valeur1"')

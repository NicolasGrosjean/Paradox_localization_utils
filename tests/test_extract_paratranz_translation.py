...
class TestExtractParatranzTranslation(unittest.TestCase):
    ...
    def test_extract_paratranz_translation_with_non_translation(self):
        ...
        extract_paratranz_localisation(os.path.join(self.data_dir, 'new_text3_l_french.yml.json'),
                                       os.path.join(self.localisation_dir, 'new_text3_l_french.yml'),
                                       False)
        with open(os.path.abspath(os.path.join(self.data_dir, 'target', 'new_text3_l_french.yml')), 'r',
                  encoding='utf8') as f:
            lines = f.readlines()
        self.assertEqual(lines[0].replace('\n', ''), 'l_french:')
        self.assertEqual(lines[1].replace('\n', ''), ' KEY0:0 "valeur0"')
    ...
    def test_extract_paratranz_translation_with_translation_not_reviewed(self):
        ...
        extract_paratranz_localisation(os.path.join(self.data_dir, 'new_text4_l_french.yml.json'),
                                       os.path.join(self.localisation_dir, 'new_text4_l_french.yml'),
                                       True)
        with open(os.path.abspath(os.path.join(self.data_dir, 'target', 'new_text4_l_french.yml')), 'r',
                  encoding='utf8') as f:
            lines = f.readlines()
        self.assertEqual(lines[0].replace('\n', ''), 'l_french:')
        self.assertEqual(lines[1].replace('\n', ''), ' KEY0:0 "valeur0"')
